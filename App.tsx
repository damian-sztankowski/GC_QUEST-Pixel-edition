    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white pixel-font p-6 overflow-hidden">
        <div className="pixel-box p-12 flex flex-col items-center border-8">
          ...
          <div className="text-2xl animate-pulse mb-6 text-yellow-500">{initStage}...</div>
          ...
          <div className="text-[10px] text-slate-500 tracking-widest text-center uppercase">
  ...
  return (
    <Layout activeRole={selectedRole}>
        ...
        <div className="relative w-full min-h-[75vh] flex flex-col items-center justify-center py-12 px-6">
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="mb-16 relative">
              ...
              <div className="text-2xl pixel-font text-yellow-500 animate-pulse tracking-widest bg-blue-900/40 inline-block px-8 py-2 border-2 border-blue-500">
                :: PIXEL_EDITION ::
              </div>
            </div>
            ...
            <div className="mb-16 max-w-2xl">
              <div className="pixel-box p-10 bg-black/90 border-4">
                 <p className="mono-font text-xl md:text-2xl text-slate-200 leading-relaxed uppercase">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-12 mb-24 w-full max-w-2xl">
              <button 
                onClick={() => setGameState(GameState.ROLE_SELECTION)}
                className="pixel-button pixel-button-primary px-20 py-10 pixel-font text-3xl group shadow-[10px_10px_0_#000]"
              >
                1P_START
              </button>
              ...
              <button 
                ...
                className="pixel-button bg-black text-white px-16 py-8 pixel-font text-xl hover:bg-slate-900"
              >
                HI_SCORE
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-5xl">
               <div className="pixel-box p-10 border-4 hover:bg-blue-950 transition-colors group">
                  ...
                  <h3 className="text-2xl pixel-font text-blue-400 mb-4">EXAM_LOGIC</h3>
                  <p className="mono-font text-slate-400 text-xl uppercase">All 6 official sections of the CDL Guide included.</p>
               </div>
               <div className="pixel-box p-10 border-4 hover:bg-red-950 transition-colors group">
                  <div className="text-5xl mb-6 group-hover:animate-pixel-float">💣</div>
                  <h3 className="text-2xl pixel-font text-red-400 mb-4">ARCADE_SPEED</h3>
                  <p className="mono-font text-slate-400 text-xl uppercase">Complete the migration before stability drops to 0%.</p>
               </div>
            </div>
          </div>
        </div>
      ...
      {gameState === GameState.ROLE_SELECTION && (
        <div className="w-full animate-in slide-in-from-bottom-8 duration-500 pt-10 px-4 flex flex-col items-center">
          <div className="text-center mb-16">
            <h2 className="text-6xl font-black pixel-font text-white mb-6">HERO_SELECT</h2>
            <div className="pixel-hr w-64 mx-auto mb-6"></div>
            <p className="text-yellow-500 pixel-font text-sm animate-pulse uppercase tracking-widest">DETECTING_USER_BIOMETRICS...</p>
          </div>
          
          <div className="max-w-2xl mx-auto w-full px-6">
            {rolesWithAvatars.map((role, idx) => (
              <RoleCard key={idx} role={role} onSelect={(r) => { setSelectedRole(r); setGameState(GameState.PLAYING); }} index={idx} />
            ))}
          </div>
          
          <button onClick={() => setGameState(GameState.HOME)} className="mt-20 pixel-button bg-black text-slate-400 px-10 py-5 pixel-font text-xs">
      ...
      {gameState === GameState.PLAYING && selectedRole && (
        <GameSessionUI role={selectedRole} onGameEnd={(s) => { setFinalScore(s); setGameState(GameState.GAME_OVER); }} />
      )}
      ...
      {gameState === GameState.GAME_OVER && (
        <div className="w-full max-w-4xl mx-auto pixel-box border-8 p-16 bg-[#0c0c0c] animate-in zoom-in-95 duration-500 text-center shadow-[16px_16px_0_#000]">
          <div className="text-7xl pixel-font text-yellow-500 mb-10 animate-pixel-float tracking-tighter">MISSION_COMPLETE!</div>
          ...
          <div className="flex flex-col items-center mb-12">
            ...
            <div className="pixel-font text-blue-400 text-3xl uppercase tracking-widest">{selectedRole}</div>
          ...
          <div className="bg-[#111] p-12 border-4 border-white mb-16 shadow-inner relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-10 justify-center">
            <button 
              onClick={resetGame}
              className="pixel-button pixel-button-primary px-16 py-8 pixel-font text-2xl shadow-[8px_8px_0_#000]"
            >
              REPLAY?
            </button>
            <button 
              onClick={() => setGameState(GameState.LEADERBOARD)}
              className="pixel-button bg-slate-800 text-white px-16 py-8 pixel-font text-2xl shadow-[8px_8px_0_#000]"
            >
              HALL_OF_FAME
            </button>
          </div>
      ...
      {gameState === GameState.LEADERBOARD && (
        <div className="w-full flex flex-col items-center pt-10 px-4 pb-20">
           <Leaderboard />
           <button onClick={() => setGameState(GameState.HOME)} className="mt-16 pixel-button bg-black text-slate-400 px-12 py-6 pixel-font text-sm">
              [ RETURN_TO_HQ ]
           </button>
        </div>
      )}
